...
<script type="text/javascript">
	$(document).ready(function(){
		var type = top.location.href.split("?")[1].split("=")[1].split("#")[0];
		if(type=="0"){
			$("#moreTypeTag").text("最新帖");
		} else {
			$("#moreTypeTag").text("最热帖");
		}
		...
		if(type=="0"){
			var url = $("#bbsMoreNewSubjectList").attr("action");
			$.post(url, {async:false}, bbsMoreNewSubjectListCallBack, "json");
		} else {
			var url = $("#bbsMoreHotSubjectList").attr("action");
			$.post(url, {async:false}, bbsMoreHotSubjectListCallBack, "json");
		}		
	});
	...
	//插入更多最新帖列表
	function bbsMoreNewSubjectListCallBack(data){
		$("#currentPageTag").text(data.currentPage);
		$("#totalPageTag").text(data.totalPage);
		$("#transactionCnt ul li").remove();
		...
		$("#transactionCnt ul").eq(0).append(str);
		check();	
	}

	//插入更多最热帖列表
	function bbsMoreHotSubjectListCallBack(data){
		$("#currentPageTag").text(data.currentPage);
		$("#totalPageTag").text(data.totalPage);
		$("#transactionCnt ul li").remove();
    ...
    function prePage(){
    	...
    	if($("#moreTypeTag").text() == "最新帖"){
        	var url = $("#bbsMoreNewSubjectList").attr("action");
			$.post(url, {currentPage:currentPageValue,async:false}, bbsMoreNewSubjectListCallBack, "json");			
        } else {
			var url = $("#bbsMoreHotSubjectList").attr("action");
			$.post(url, {currentPage:currentPageValue,async:false}, bbsMoreHotSubjectListCallBack, "json");
        }
    }
    ...
    <div id="content" class="CENTER">
    	<div id="left">
            ...
            <div id="left1Cnt" class="DIVCNT">
               ...
               <input id="searchButton" onclick="bbsSubjectSearch();" type="button" value="搜索" class="BUTTON"/>
            </div>
        ...
        <div id="right">
            <div id="transaction" class="DIVTITLE">
                <span class="TITLELEFTSPAN">&nbsp;当前位置：<a href="../../home.jsp">首页</a>><a href="forumHome.jsp">论坛首页</a>><span id="moreTypeTag" class="TITLELEFTSPAN"></span></span>
            </div>
            ...
            <div id="foot">
            	<ul>
                    ...
                    <li>第<span id="currentPageTag"></span>页</li>
                    <li>共<span id="totalPageTag"></span>页</li>
                </ul>
            </div> 
        </div>
    </div>
