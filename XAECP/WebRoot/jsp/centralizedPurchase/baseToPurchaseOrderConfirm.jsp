...
<script language="javascript">
	...
	function sendGoods(valu){
		...
		var purchaseOrderIdV=$(valu).attr("name");
		var url="assureSendGoods.action";
		var addressIdV=""
		var productsV="";
		$("input[name='selectAddress']").each(function(){
		...
		$("tr[id='productList']").each(function(){
			productsV=productsV+$(this).children("td").eq(0).text()+","+$(this).children("td").eq(3).text()+"@";
		});
		var params={purchaseOrderId:purchaseOrderIdV,
					logisticsSupplierId:logisticsSupplierIdV,
					logisticsOrderId:logisticsOrderIdV,
					addressId:addressIdV,
					products:productsV};
		$.post(url,params,function(data){
			alert("确认发货成功！");
...
<body>
<form id="toFrontpageForm" action="listOrders.action"></form>
      ...
      <tr>
        ...
        <table id="registCnt" style="margin-top:15px; margin-bottom:10px;" class="CENTER" cellpadding="0" cellspacing="0" align="center">
          <tr>
            ...
            <td class="ALLEFT"><s:property value="purchaseOrder.purchaseOrderId"/></td>
            <td class="ALRIGHT">供应商名称</td>
            <td class="ALLEFT"><s:property value="purchaseOrder.purchaserName"/></td>
          </tr>
          <tr>
            <td class="ALRIGHT">集中采购信息代码</td>
            <td class="ALLEFT"><s:property value="purchaseOrder.CPIId"/> </td>
            <td class="ALRIGHT">计划采购信息代码</td>
            <td class="ALLEFT"></tr>
          <tr>
            <td class="ALRIGHT">交货日期</td>
            <td class="ALLEFT"><s:property value="purchaseOrder.deliverDate"/></td>
            <td class="ALRIGHT">应支付定金</td>
            <td class="ALLEFT"><s:property value="purchaseOrder.ownOrder"/>（元）</td>
          </tr>
          <tr>
            <td class="ALRIGHT">运费</td>
            <td class="ALLEFT"><s:property value="purchaseOrder.fare"/>（元）</td>
            <td class="ALRIGHT">总货款</td>
            <td class="ALLEFT"><s:property value="purchaseOrder.totalPrice"/>（元）</td>
          </tr>
          <tr>
          	<td class="ALRIGHT">收货地址</td>
            <td class="ALLEFT" colspan="3"><s:property value="purchaseOrder.receiveAddress"/></td>
          </tr>
        </table></td>
      </tr>
      <tr>
      ...
      <tr>
      	<td>
        	<table class="CENTER">
                ...
                <tr align="center">
                	<td><input type="button" name="<s:property value="purchaseOrder.purchaseOrderId"/>" onclick="sendGoods(this)" class="BIGREDBUTTON" value="确认发货" /></td>
                </tr>
            </table>
        </td>
      </tr>
